...
import sys
from .common import *
# Just importing this enables readline when doing input:
import readline
...
class FileTokenizer:
    ...
    def __init__(self, f, output=None, prompt=None):
        # Python 3 compat: check for file-like object with 'read' method
        # (In Python 2 this was `type(f) is file`)
        if hasattr(f, 'read') and not isinstance(f, TrackingStream):
            f = TrackingStream(f)
        self.file = f
